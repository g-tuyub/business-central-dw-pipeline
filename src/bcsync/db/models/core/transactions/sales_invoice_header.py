...
class SalesInvoiceHeader(CoreBase):
    ...
    sell_to_customer_id: Mapped[int] = mapped_column(Integer, ForeignKey(f'{DBSchemas.CORE}.customer.id'), nullable=True, index=True)
    bill_to_customer_code: Mapped[str] = mapped_column(String(20), nullable=True)
    bill_to_customer_id: Mapped[int] = mapped_column(Integer, ForeignKey(f'{DBSchemas.CORE}.customer.id'), nullable=True, index=True)
    ship_to_address_code: Mapped[str] = mapped_column(String(20), nullable=True)
    ship_to_address_id: Mapped[int] = mapped_column(Integer, ForeignKey(f'{DBSchemas.CORE}.ship_to_address.id'), nullable=True, index=True)
    ...
    customer_ledger_entry_id: Mapped[int] = mapped_column(Integer, ForeignKey(f'{DBSchemas.CORE}.customer_ledger_entry.id'), nullable=True, index=True)
