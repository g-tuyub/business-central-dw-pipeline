...
- Creación automática del esquema de la base de datos, incluyendo *stored procedures* y *views*.
...
- **SQLAlchemy** (**ORM** y conexión a SQL Server)
- **Pydantic** (validación de datos y configuración)
- **Microsoft SQL Server** (base de datos destino)
- **Dynamics 365 Business Central Custom APIs** (Extensión AL personalizada)
...
* **Azure Active Directory (Entra ID):**
    * Se requiere un **App Registration** en Azure para la autenticación **OAuth2.0** (Service-to-Service).
...
* **Dynamics 365 Business Central:**
    * **Dependencia:** La extensión `.app` que expone la **API** debe estar instalada y activa en el entorno.
    ...
    * Asignar el *Permission Set* al registro de aplicación correspondiente.
...
python -m venv .venv        #crear entorno virtual y activar
...
Primero asegúrate de crear la base de datos (debe coincidir con la variable DB_DATABASE que configuraste en el archivo *.env*):
...
#### Entorno Existente:
Si tu archivo *.env* apunta a una base de datos donde el **Data Warehouse** ya está implementado, no necesitas ejecutar este paso inicial, salvo que hayas agregado nuevas tablas o modificado procedimientos y necesites aplicar los cambios.
...
```
Si utilizas un servidor remoto, reemplazarías **localhost** por la **IP** o dominio correspondiente.
...
```
Este comando ejecutará una sincronización de la tabla: *customer*.
